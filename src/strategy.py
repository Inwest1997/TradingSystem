...
def rsi_strategy(df,up=50, down=10, **kwrgs):
    df = pd.concat([df,rsi(df)], axis =1)
    df["trade"] = [-1 if df.iloc[i]['RSI']>up and df.iloc[i-1]['RSI'] < up else 1 if df.iloc[i]['RSI']<down and df.iloc[i-1]['RSI'] > down else 0 for i in range(len(df))] 
    buy =0
    position=[]
...
def macd_strategy(df, **kwrgs):
    ...
    df["trade"] = [-1 if df.iloc[i]['macd_oscillator'] < 0 and df.iloc[i-1]['macd_oscillator'] > 0  else 1 if df.iloc[i]['macd_oscillator']> 0 and df.iloc[i-1]['macd_oscillator'] < 0 else 0 for i in range(len(df))] 
    buy =0
    position=[]
    for i in range(len(df)):
        if i ==len(df)-1:
...
def envelope_strategy(df, **kwrgs):
    ...
    df["trade"] = [-1 if df.iloc[i]['Adj Close'] > df.iloc[i]['en_ub'] and df.iloc[i-1]['Adj Close'] < df.iloc[i-1]['en_ub'] else 1 
                   if df.iloc[i]['Adj Close']<df.iloc[i]['en_lb'] and df.iloc[i-1]['Adj Close']>df.iloc[i-1]['en_lb'] else 0 for i in range(len(df))] 
    buy =0
    position=[]
    for i in range(len(df)):
        if i ==len(df)-1:
...
def bollinger_strategy(df, **kwrgs):
    ...
    df["trade"] = [-1 if df.iloc[i]['Adj Close'] > df.iloc[i]['bo_lb'] and df.iloc[i-1]['Adj Close'] < df.iloc[i-1]['bo_lb'] else 1 
                  if df.iloc[i]['Adj Close']<df.iloc[i]['bo_lb'] and df.iloc[i-1]['Adj Close'] > df.iloc[i-1]['bo_lb'] else 0 for i in range(len(df))] 
    buy =0
    position=[]
    for i in range(len(df)):
        if i ==len(df)-1:
...
def stochastic_strategy(df, **kwrgs):
    ...
    df["trade"] = [-1 if df.iloc[i]['fast_k'] > df.iloc[i]['slow_k'] and df.iloc[i-1]['fast_k'] < df.iloc[i-1]['slow_k'] else 1 
                     if df.iloc[i]['fast_k']<df.iloc[i]['slow_k'] and df.iloc[i-1]['fast_k']>df.iloc[i-1]['slow_k'] else 0 for i in range(len(df))]     
    buy =0
    position=[]
    for i in range(len(df)):
        if i ==len(df)-1:
...
def cci_strategy(df, up =70 ,down = 20, **kwrgs):
    df = pd.concat([df ,cci(df, **kwrgs)], axis =1)

    df["trade"] = [-1 if df.iloc[i]['CCI'] > up and df.iloc[i-1]['CCI'] < up else 1 
                    if df.iloc[i]['CCI'] < down and df.iloc[i-1]['CCI'] > down else 0 for i in range(len(df))] 
    buy =0
    position=[]
    for i in range(len(df)):
        if i ==len(df)-1:
...
def test(df, *args):
    ...
    for i in range(len(df)):
        if df.iloc[i]['position'] == 1:
            ...
            position.append(0)
        elif df.iloc[i]['Adj Close'] / price > 1.07 or df.iloc[i]['Adj Close'] / price < 0.97:
            price=0
            position.append(-1)
        else:
            price=0
...
#                 buy=1
#                 position.append(1)
#             elif df.iloc[i]['position'] == 0 and buy ==1:
#                 position.append(10)
#             elif df.iloc[i]['position'] == 0 and buy ==0 or df.iloc[i]['position'] == -1:
#                 position.append(0)
#             else: 
#                 buy=0
#                 position.append(-1)
